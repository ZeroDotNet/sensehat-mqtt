...
def on_subscribe(client, userdata, flags, rc):
    if type(rc) == int:
        if rc == 0:
            ...
            print(flags)
            mqtt_client.publish(cfg["topics"]["pressure"] + cfg["topics"]["config"], json.dumps(sensehat_device.message_config_pressure("pressure")), qos=1, retain=True)
            print(
                 "[on_connect] Published to "
                 + cfg["topics"]["pressure"]
                 + cfg["topics"]["config"]
...
def publish(client: Client, topic: str, msg: str, retain: bool = False, qos: int = 0) -> MQTTMessageInfo:
    msg_count = 1
    while True:
        result = client.publish(topic, msg, retain=retain, qos=qos)

        if result.rc == 0 or result.is_published():
            print(f"[publish] Message {result.mid}: `{msg}` to topic `{topic}` published.")
            return result
        print(f"[publish] Failed to send message to topic {topic}. Message: {result.mid} - Code: {result.rc}")
        msg_count += 1
        if msg_count > 5:
            print(f"[publish] Timeout while retrying... #{msg_count} - Message: {result.mid} - Code: {result.rc}")
            break
        print(f"[publish] Retrying... #{msg_count} - Message: {result.mid} - Code: {result.rc}")
        time.sleep(1)
    return result
...
def build_mqtt() -> Client:
    ...
    # Generate a ClientId
    client_id_alt = str(cfg["id"])
    build_client = Client(client_id=client_id_alt, clean_session=True, reconnect_on_failure=True)
    username = "admin"
    password = "majestic"
...
def mqtt_connect(param_client: Client):
    ...
    param_client.connect(cfg["broker"]["host"], port=cfg["broker"]["port"], keepalive=60, clean_start=MQTT_CLEAN_START_FIRST_ONLY)
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            json_read = metrics.get("messages", "").get("json", "")
        ...
        sys.stderr.write(f"Traceback: {ex.traceback.format_exc()}\n")
        exit_code = 1
    finally:
        # client.disconnect()
